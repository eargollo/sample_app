...
class ForgotPasswordFormTest < PasswordResets
  ...
  test "password reset path" do
    get passwords_resets_new_path
    assert_template 'passwords_resets/new'
    assert_select 'input[name=?]', 'passwords_reset[email]'
  end

  test "reset path with invalid email" do
    post passwords_resets_path, params: { passwords_reset: { email: "" } }
...
class PasswordResetForm < PasswordResets
  def setup
    super
    @user = users(:michael)
    post passwords_resets_path,
      params: { passwords_reset: { email: @user.email } }
...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with wrong email" do
    get edit_passwords_reset_path(@reset_user.reset_token, email: "")
    assert_redirected_to root_url
  end

  test "reset with inactive user" do
    @reset_user.toggle!(:activated)
    get edit_passwords_reset_path(@reset_user.reset_token,
      email: @reset_user.email)
    assert_redirected_to root_url
  end

  test "reset with right email but wrong token" do
    get edit_passwords_reset_path('wrong token', email: @reset_user.email)
    assert_redirected_to root_url
  end
  ...
  test "reset with right email and right token" do
    get edit_passwords_reset_path(@reset_user.reset_token,
      email: @reset_user.email)
    assert_template 'passwords_resets/edit'
...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with invalid password and confirmation" do
    patch passwords_reset_path(@reset_user.reset_token),
